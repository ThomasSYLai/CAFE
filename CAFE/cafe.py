    ...
    while acceptFit is False:
        ...
        try:
            result = fitter.minimize(
                method=method, ftol=ftol, max_nfev=200 * (len(params) + 1)
            )
        except:
...
class cubemod:
    ...
    def fit_cube(
        ...
        wave, flux, flux_unc, bandname, mask = mask_spec(
            self, ind_seq[1][0], ind_seq[0][0]
        )
        spec = Spectrum1D(
    ...
    def plot_cube_fit(self, x, y, inparfile, optfile, savefig=None):
        ...
        if hasattr(self, "parcube") is False:
            ...
            CompFluxes, CompFluxes_0, extComps, e0, tau0, vgrad = (
                get_model_fluxes(params, wave, cont_profs, comps=True)
            )
...
class specmod:
    ...
    def read_spec(
        ...
        try:
            cube = cafeio.read_cretacube(
                file_dir + file_name, flux_key, flux_unc_key
            )
        except:
            hdu = fits.PrimaryHDU()
            ...
            try:
                ...
                try:
                    ...
                    if read_columns != None:
                        ...
                        if extension == ".csv":
                            ...
                            if (
                                ...
                                raise IOError(
                                    "Only the CAFE produced csv file can be ingested."
                                )
                    ...
                    raise IOError(
                        "The file is not a valid .txt (column-based) or .fits (CRETA output) file. Or maybe the data are not there."
                    )
                else:
                    spec = Spectrum1D(
        ...
        minWave = (
            wave_min
            if wave_min is not None
            else np.nanmin(cube.waves[val_inds])
        )
        maxWave = (
            wave_max
            if wave_max is not None
            else np.nanmax(cube.waves[val_inds])
        )
        minWave = (
            rwave_min * (1 + z)
        ...
        fit_wave_inds = (cube.waves[val_inds] >= minWave) & (
            cube.waves[val_inds] <= maxWave
        )
        ...
        log_f_med = np.log10(np.nanmedian(self.fluxes))
    ...
    def fit_spec(
        ...
        if self.init_parcube is not None:
            ...
            params = param_gen.make_parobj(
                parobj_update=cube_params, get_all=True, init_parobj=init_params
            )
        else:
            params = init_params
        ...
        # Get compdict
        CompFluxes, CompFluxes_0, extComps, e0, tau0, vgrad = get_model_fluxes(
            self.params, self.spec_dict["wave"], self.cont_profs, comps=True
        )
    ...
    def plot_spec_ini(self):
        ...
        if self.init_parcube is not None:
            ...
            params = param_gen.make_parobj(
                parobj_update=cube_params, get_all=True
            )
    ...
    def save_result(
        ...
        if asdf is True:
            ...
            CompFluxes, CompFluxes_0, extComps, e0, tau0, _ = get_model_fluxes(
                params, wave, self.cont_profs, comps=True
            )
